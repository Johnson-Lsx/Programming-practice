...
Polynomial Polynomial::dif2()const
{
	Polynomial dif1(dif());
	Polynomial dif2(dif1.dif());
	return dif2;
}

string Polynomial::calc(double x)const
{
	double res = 0;
	int len = poly.size();
	if (len == 0)
...
double Polynomial::calcu(double x)const
{
	double res = 0;
	int len = poly.size();
	if (len == 0)
...
bool Polynomial::checkNewton(double a, double b) const
{
	if (calcu(a) * calcu(b) >= 0)
		return false;
	Polynomial x(dif()),y(dif2());//不准确
	if (x.calcu(a) * x.calcu(b) < 0)
		return false;
	if (y.calcu(a) * y.calcu(b) < 0)//不准确
		return false;
	//if (a - calcu(a) / x.calcu(a) > b || b - calcu(b) / x.calcu(b) < a)
		//return false;
	if (a > b)
		return false;
	return true;
...
double Polynomial::Newton(double x) const
{
	double x1, x2;
	Polynomial y = dif();
	x1 = x;
	do {
		x2 = x1 - calcu(x1) / y.calcu(x1);
		double tmp;
		tmp = x1;
		x1 = x2;
		x2 = tmp;
